import pygame
import math

# Inicialização do Pygame
pygame.init()

# Configurações da tela
screen_width, screen_height = 800, 600
screen = pygame.display.set_mode((screen_width, screen_height))
pygame.display.set_caption("Batalha de Tanques")

# Carregar e redimensionar o cenário para o tamanho da tela
background_image = pygame.image.load("C:/Users/jorja/Downloads/GRAMA.JPG").convert()
background_image = pygame.transform.scale(background_image, (screen_width, screen_height))

# Classe do tanque
class Tank(pygame.sprite.Sprite):
    def __init__(self, name, x, y):
        super().__init__()
        self.original_image = pygame.image.load("C:/Users/jorja/Downloads/klipartz.com1.png").convert_alpha()
        self.image = pygame.transform.scale(self.original_image, (100, 60))  # Redimensiona a imagem
        self.rect = self.image.get_rect(center=(x, y))
        self.name = name
        self.angle = 0
        self.speed = 5

    def update(self, keys):
        if keys[pygame.K_LEFT]:
            self.angle += 5
        if keys[pygame.K_RIGHT]:
            self.angle -= 5

        # Atualiza a posição com base na direção
        if keys[pygame.K_UP]:
            self.rect.x += self.speed * math.cos(math.radians(-self.angle))
            self.rect.y += self.speed * math.sin(math.radians(-self.angle))
        if keys[pygame.K_DOWN]:
            self.rect.x -= self.speed * math.cos(math.radians(-self.angle))
            self.rect.y -= self.speed * math.sin(math.radians(-self.angle))

        # Verifica se o tanque saiu da tela e ajusta a posição
        if self.rect.x < -220:
            self.rect.x = screen_width
        elif self.rect.x > screen_width:
            self.rect.x = 0

        if self.rect.y < -220:
            self.rect.y = screen_height
        elif self.rect.y > screen_height:
            self.rect.y = 0

        # Rotaciona a imagem do tanque
        self.image = pygame.transform.rotate(self.original_image, self.angle)
        self.rect = self.image.get_rect(center=self.rect.center)

    def draw(self, screen):
        screen.blit(self.image, self.rect)

    def fire(self, bullets):
        # Função de disparo, cria um novo projétil
        bullet = Bullet(self.rect.centerx, self.rect.centery, self.angle)
        bullets.add(bullet)

# Classe do projétil
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, angle):
        super().__init__()
        self.image = pygame.image.load("C:/Users/jorja/Downloads/CIRCULO.png").convert_alpha()
        self.image = pygame.transform.scale(self.image, (15, 15))  # Redimensiona o projétil
        self.rect = self.image.get_rect(center=(x, y))
        self.angle = angle
        self.speed = 10

    def update(self):
        # Atualiza a posição do projétil
        self.rect.x += self.speed * math.cos(math.radians(-self.angle))
        self.rect.y += self.speed * math.sin(math.radians(-self.angle))

        # Remove o projétil se ele sair da tela
        if self.rect.x < 0 or self.rect.x > screen_width or self.rect.y < 0 or self.rect.y > screen_height:
            self.kill()

# Criando o tanque
tank = Tank("Panzer III", screen_width // 2, screen_height // 2)

# Grupo de sprites para os projéteis
bullets = pygame.sprite.Group()

# Loop principal
running = True
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False

    keys = pygame.key.get_pressed()  # Captura as teclas pressionadas
    tank.update(keys)  # Atualiza a posição do tanque

    if keys[pygame.K_SPACE]:  # Verifica se a tecla espaço está pressionada para atirar
        tank.fire(bullets)

    bullets.update()  # Atualiza as posições dos projéteis

    # Desenhando o cenário
    screen.blit(background_image, (0, 0))

    # Desenhando o tanque e os projéteis sobre o cenário
    tank.draw(screen)
    bullets.draw(screen)

    pygame.display.flip()
    pygame.time.delay(30)

pygame.quit()
